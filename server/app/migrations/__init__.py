...
Run once on the production database:
  python -m app.migrations.chat_memory_upgrade
"""
import asyncio
...
async def migrate():
    async with engine.begin() as conn:
        ...
        # Drop old position column and index (if exists)
        await conn.execute(text("""
            DROP INDEX IF EXISTS ix_messages_conv_position
        """))
        await conn.execute(text("""
            ALTER TABLE messages DROP COLUMN IF EXISTS position
        """))
